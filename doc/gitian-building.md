...
*Setup instructions for a gitian build of COTN coin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the COTN coin
...
- [Building COTN coin](#building-cottoncoin)
...
Clone the git repositories for cottoncoin and gitian and then checkout the cottoncoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the cottoncoin repository
...
To build COTN coin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the cottoncoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/cottoncoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/cottoncoin/cottoncoin
...
For example:
```bash
URL=https://github.com/konez2k/cottoncoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit cottoncoin=${COMMIT} --url cottoncoin=${URL} ../cottoncoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit cottoncoin=${COMMIT} --url cottoncoin=${URL} ../cottoncoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit cottoncoin=${COMMIT} --url cottoncoin=${URL} ../cottoncoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/cottoncoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/cottoncoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/cottoncoin-build.assert
```
...
[cottoncoin/gitian.sigs](https://github.com/cottoncoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
