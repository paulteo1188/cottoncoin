...
COTN coin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<cottoncoin_address\> \<monthly_payment_cottoncoin\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ COTN, _12_ times over the course of a year totaling _24000_ COTN.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<cottoncoin_address\> \<monthly_payment_cottoncoin\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` COTN.
 ...
 * vote-many          - Vote on a COTN coin initiative
 * vote-alias         - Vote on a COTN coin initiative
 * vote               - Vote on a COTN coin initiative/budget
